...
export function FallbackArena2D({
  ...
  // Brawl-Stars-style aim guide elements — glow band, bright core, charge
  // fill, marching dashes, comet, arrowhead + end ring, area/heal rings and
  // a red auto-aim reticle on the enemy
  const aimGlow = useRef<SVGLineElement>(null);
  const aimCore = useRef<SVGLineElement>(null);
  const aimCharge = useRef<SVGLineElement>(null);
  const aimDash = useRef<SVGLineElement>(null);
  ...
  const aimReticle = useRef<SVGGElement>(null);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // Brawl-Stars-style aim guide — a bright directional beam that stops at
      // obstacles, fills up as the next shot charges, has marching dashes,
      // an arrowhead + end ring at the impact point, and a red auto-aim
      // reticle on the enemy. Direction follows the attack-joystick drag;
      // without a drag it auto-aims at the enemy, like Brawl Stars.
      ...
      // The guide stops at the first obstacle the shot would hit, so what
      // you see is exactly where the projectile will stop.
      const hitRange =
        arange > 0 ? raycastObstacleRange(p.x, p.y, aux, auy, arange) : 0;
      const YOFF = 28; // lift the guide up to the fighter's "hands"
      const sx = p.x;
      const sy = p.y - YOFF;
      const esx = p.x + aux * hitRange;
      const esy = p.y + auy * hitRange - YOFF;
      // Next-shot charge (0 → 1 while the cooldown counts down). While
      // dashing the attack is locked, so the guide just shows it ready.
      const load =
        p.dashT > 0 ? 1 : Math.max(0, Math.min(1, 1 - p.atkCd / ATK_CD));
      const ready = load >= 1;
      const readyPop = ready ? 1 + 0.1 * Math.sin(elapsed * 18) : 1;
      const lineShown = hitRange > 0 && aiming;
      const angDeg = (Math.atan2(auy, aux) * 180) / Math.PI;
      const glow = 0.7 + 0.3 * Math.sin(elapsed * 7);
      const setLine = (
        ln: SVGLineElement | null,
        x1: number,
        y1: number,
        x2: number,
        y2: number,
        ...
        ln.setAttribute("opacity", `${opacity.toFixed(2)}`);
      };
      const hideLine = (ln: SVGLineElement | null) =>
        ln?.setAttribute("visibility", "hidden");

      if (lineShown) {
        // soft glow band under the beam so it pops against the grass
        setLine(aimGlow.current, sx, sy, esx, esy, acolor, 0.13 + 0.05 * glow);
        // bright core beam to the impact point
        setLine(aimCore.current, sx, sy, esx, esy, acolor, 0.7 + 0.25 * glow);
        // charge fill — the loaded segment grows toward the tip, then the
        // shot fires and it snaps back (readable hold-to-fire timing)
        const cxp = p.x + aux * hitRange * load;
        const cyp = p.y + auy * hitRange * load - YOFF;
        if (load > 0.01) {
          setLine(aimCharge.current, sx, sy, cxp, cyp, "#ffffff", 0.9 * readyPop);
        } else {
          hideLine(aimCharge.current);
        }
        // marching dashes flow player → tip (directional flow)
        if (aimDash.current) {
          aimDash.current.setAttribute("visibility", "visible");
          aimDash.current.setAttribute("x1", `${sx}`);
          aimDash.current.setAttribute("y1", `${sy}`);
          aimDash.current.setAttribute("x2", `${esx}`);
          aimDash.current.setAttribute("y2", `${esy}`);
          aimDash.current.setAttribute("stroke", acolor);
          aimDash.current.setAttribute("stroke-dashoffset", `${-(elapsed * 170)}`);
          aimDash.current.setAttribute(
            "opacity",
            `${(0.55 + 0.25 * glow).toFixed(2)}`,
          );
        }
        if (aimComet.current) {
          const prog = (elapsed * 0.9) % 1;
          aimComet.current.setAttribute("visibility", "visible");
          aimComet.current.setAttribute("cx", `${sx + aux * hitRange * prog}`);
          aimComet.current.setAttribute("cy", `${sy + auy * hitRange * prog}`);
          aimComet.current.setAttribute(
            "r",
            `${(10 + 3 * Math.sin(elapsed * 12)) * readyPop}`,
          );
          aimComet.current.setAttribute("fill", acolor);
          aimComet.current.setAttribute("opacity", "0.95");
        }
        if (aimArrow.current) {
          aimArrow.current.setAttribute("visibility", "visible");
          aimArrow.current.setAttribute(
            "transform",
            `translate(${esx} ${esy}) rotate(${angDeg}) scale(${readyPop.toFixed(3)})`,
          );
          aimArrow.current.setAttribute("fill", acolor);
          aimArrow.current.setAttribute(
            "opacity",
            `${(0.85 + 0.15 * glow).toFixed(2)}`,
          );
        }
        if (aimEnd.current) {
          aimEnd.current.setAttribute("visibility", "visible");
          aimEnd.current.setAttribute("cx", `${esx}`);
          aimEnd.current.setAttribute("cy", `${esy}`);
          aimEnd.current.setAttribute(
            "r",
            `${(18 + 4 * Math.sin(elapsed * 6)) * readyPop}`,
          );
          aimEnd.current.setAttribute("stroke", acolor);
          aimEnd.current.setAttribute("fill", `${acolor}2b`);
          aimEnd.current.setAttribute(
            "opacity",
            `${(0.5 + 0.3 * glow).toFixed(2)}`,
          );
        }
      } else {
        hideLine(aimGlow.current);
        hideLine(aimCore.current);
        hideLine(aimCharge.current);
        hideLine(aimDash.current);
        aimComet.current?.setAttribute("visibility", "hidden");
        aimArrow.current?.setAttribute("visibility", "hidden");
        aimEnd.current?.setAttribute("visibility", "hidden");
      }
      // explosion area (fireball) — pulsing ring at the impact point
      if (aimArea.current) {
        if (aarea > 0 && aiming) {
          aimArea.current.setAttribute("visibility", "visible");
          aimArea.current.setAttribute("cx", `${esx}`);
          aimArea.current.setAttribute("cy", `${esy}`);
          aimArea.current.setAttribute("r", `${130 + 8 * Math.sin(elapsed * 4)}`);
          aimArea.current.setAttribute(
            "opacity",
            `${(0.5 + 0.25 * Math.sin(elapsed * 5)).toFixed(2)}`,
          );
        } else {
          aimArea.current.setAttribute("visibility", "hidden");
        }
      }
      // self-heal ring around the player
      if (aimHeal.current) {
        if (ability === "sifa" && aiming) {
          aimHeal.current.setAttribute("visibility", "visible");
          aimHeal.current.setAttribute("cx", `${p.x}`);
          aimHeal.current.setAttribute("cy", `${p.y}`);
          aimHeal.current.setAttribute(
            "opacity",
            `${(0.5 + 0.25 * Math.sin(elapsed * 5)).toFixed(2)}`,
          );
        } else {
          aimHeal.current.setAttribute("visibility", "hidden");
        }
      }
      // red auto-aim reticle on the enemy — tap-attack locks onto it
      const reticleOn = aiming && !manual && arange > 0;
      if (aimReticle.current) {
        aimReticle.current.setAttribute(
          "visibility",
          reticleOn ? "visible" : "hidden",
        );
        if (reticleOn) {
          const rs = 1 + 0.06 * Math.sin(elapsed * 7);
          aimReticle.current.setAttribute(
            "transform",
            `translate(${b.x} ${b.y - YOFF}) scale(${rs.toFixed(3)})`,
          );
        }
      }
  ...
  return (
    <div className="absolute inset-0" onClick={handleClick}>
      ...
      >
        ...
        {/* Brawl-Stars-style aim guide — glow band, bright core, charge
            fill, marching dashes, arrowhead + end ring, area/heal rings and
            a red auto-aim reticle on the enemy */}
        <g>
          <line
            ref={aimGlow}
          ...
          <line
            ...
            visibility="hidden"
          />
          <circle ref={aimComet} r="10" fill="#7dd3fc" visibility="hidden" />
          <polygon
            ref={aimArrow}
            points="-16,-15 20,0 -16,15"
            fill="#7dd3fc"
            visibility="hidden"
          />
          <circle
          ...
          <g ref={aimReticle} visibility="hidden">
            <circle
              r="34"
